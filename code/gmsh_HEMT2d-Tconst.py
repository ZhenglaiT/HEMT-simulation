...
for i in AlGaN_regions:
    ...
    CreateReferencePotential(device,i)
    CreateBandStructure(device,i)
...
for i in ("gate",):
    ...
    r = tmp[0]
    print("%s %s" % (r, i))
    CreateSchottkyContacts(device, r, i, workfun=5.5)
    # CreateSiliconPotentialOnlyContact(device, r, i)
    set_parameter(device=device, name="gate_bias", value=0.0)

# for i in ("drain_AlGaN","source_AlGaN","drain_GaN","source_GaN"):
...
for i in regions:
    ...
    CreateVaribleDerivatives(device,i)
    gradEFn = "ElectricField + edgeV_t_T*log(Electrons@n1/Electrons@n0)*EdgeInverseLength"
    # gradEFn = "ElectricField"
    CreateEdgeModel(device, i, "GEFN", gradEFn)
    CreateEdgeModel(device, i, "GEFN:Potential@n0", "ElectricField:Potential@n0")
    CreateEdgeModel(device, i, "GEFN:Potential@n1", "ElectricField:Potential@n1")
    CreateEdgeModel(device, i, "GEFN:Electrons@n0", "-edgeV_t_T*EdgeInverseLength/Electrons@n0")
    CreateEdgeModel(device, i, "GEFN:Electrons@n1", "edgeV_t_T*EdgeInverseLength/Electrons@n1")
    CreateEdgeModelDerivatives(device, i, "GEFN", gradEFn,"Temperature")
    ehfs = "GEFN"
    CaugheyThoms_electronsVelocitySaturation(device, i, "mu_vsat_e", "mu_n_Albrecht", ehfs)

# CreateAlGaNDriftDiffusion(device, "AlGaN",mu_n="mu_vsat_e", mu_p="mu_p_Albrecht")
# CreateGaNDriftDiffusion(device, "GaN",mu_n="mu_vsat_e", mu_p="mu_p_Albrecht")

CreateAlGaNDriftDiffusion(device, "AlGaN",mu_n="mu_n_Albrecht", mu_p="mu_p_Albrecht")
CreateGaNDriftDiffusion(device, "GaN",mu_n="mu_vsat_e", mu_p="mu_p_Albrecht")

for i in regions:
    CreateJouleHeatHEMT(device,i)
...
for c in ("gate",):
    ...
    CreateDriftDiffusionAtSchottkyContact(device, r, c, workfun=5.5)
...
# # for c in ("drain_AlGaN","source_AlGaN","drain_GaN","source_GaN"):
# for c in ("source",):
#     tmp = get_region_list(device=device, contact=c)
#     r = tmp[0]
#     # CreateSiliconDriftDiffusionAtContact(device, r, c)
...
for c in ("body", ):
    tmp = get_region_list(device=device, contact=c)
    r = tmp[0]
    # CreateDriftDiffusionAtSubstrate(device, r, c)
    CreateSiliconDriftDiffusionAtContact(device, r, c)      


solve(type="dc", absolute_error=1.0e10,relative_error=1e-5, maximum_iterations=1000)

write_devices(file="gmsh_HEMT2d_zeroBias", type="tecplot")
...
rampbias(device, "drain", 12, 0.01, 0.001, 100, 1e-10, 1e30, printAllCurrents)    ##current unit is A/cm
